<template>
	<view class="page-container">
		<com-page-title :title="getLanguage('我的好友')" />
		...
		:empty-view-text="getLanguage('暂时没有任何数据')"
			...
			<view class="list-box">
				<view class="item panel-item" v-for="(item,index) in list" :key="index">
					...
					<view class="info">
						<view class="name bold ell1">{{item.nickname}}</view>
						<view class="recharge">累计充值：<text>{{item.total_recharge || '0.00'}}</text></view>
						<view class="time text-gray">{{getLanguage('注册时间')}}：{{item.create_time}}</view>
					</view>
				</view>
			</view>
...
export default {
	...
	methods: {
		...
		async queryList(pageNo, pageSize) {
			...
			let result = await this.userApi.userFriends({
				page:pageNo,
				limit:pageSize
			});
