<template>
	<view class="page-container">
		<com-page-title :title="getLanguage('厂商游戏分类')" />
		<view class="category">
			<scroll-view scroll-x scroll-with-animation class="scroll-box" :scroll-into-view="`id-${currentSupplierId}`">
				<view :class="{active:currentSupplierId == item.id}" :id="`id-${item.id}`" v-for="(item,index) in supplierList" :key="index" @click="changeSupplier(item)">
...
export default {
	...
	async onLoad(options) {
		this.currentSupplierId = options.id || 0;
	...
	methods: {
		// 获取厂商列表
		async getSupplier(){
			let result = await this.gameApi.getSupplier();
			result.data.unshift({id:0,name:'全部',icon:'/static/all-supplier.png'});
